  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-[#f7f8fa] to-[#ebedfa] flex flex-col">
      ...
      <header className="w-full px-8 pt-8 flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          <div className="hidden md:flex flex-col text-right leading-tight">
            <span className="font-semibold text-zinc-700">{user.name || "Anonymous"}</span>
      ...
      <main className="flex-1 flex flex-col items-center px-4 pt-6 pb-12">
        ...
        {/* Modern: Optionally show a list of existing campaigns below (hidden/modal for modern look) */}
        {/* <div className="w-full max-w-5xl mt-12">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-zinc-800 text-lg font-bold">Existing Campaigns</h3>
            <span className="text-xs text-violet-400">Click ••• to Edit</span>
          </div>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="bg-zinc-100/80 border border-zinc-200 rounded-2xl p-6 animate-pulse h-40" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
