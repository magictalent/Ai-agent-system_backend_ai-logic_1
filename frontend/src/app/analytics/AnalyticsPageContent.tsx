...
export default function AnalyticsPageContent() {
  ...
  const [totals, setTotals] = useState<{ leads: number; outbound: number; inbound: number }>({
    leads: 0,
    outbound: 0,
    inbound: 0,
  });
    ...
    const load = async () => {
      try {
        ...
        const [tsRes, agRes] = await Promise.all([
          fetch(`${base}/dashboard/timeseries?period=${period}`, { headers }),
          fetch(`${base}/dashboard/analytics?period=${period}`, { headers }),
        ]);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1 rounded ${period === "7d" ? "bg-blue-600 text-white" : "bg-white border"}`}
          >
            Weekly
          </button>
          <button
            onClick={() => setPeriod("30d")}
            className={`px-3 py-1 rounded ${period === "30d" ? "bg-blue-600 text-white" : "bg-white border"}`}
          >
            Monthly
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Chart
          ...
          color="#2563eb"
          onClick={(d) => router.push(`/analytics?date=${encodeURIComponent(d)}&focus=leads`)}
          highlight={focus === "leads"}
        />
        <Chart
          title="Outbound"
          ...
          labels={series.map((s) => s.date)}
          color="#7c3aed"
          onClick={(d) => router.push(`/analytics?date=${encodeURIComponent(d)}&focus=outbound`)}
          highlight={focus === "outbound"}
        />
        <Chart
          title="Inbound"
          ...
          labels={series.map((s) => s.date)}
          color="#16a34a"
          onClick={(d) => router.push(`/analytics?date=${encodeURIComponent(d)}&focus=inbound`)}
          highlight={focus === "inbound"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <TableCard
          title="By Channel"
          columns={["Channel", "Outbound", "Inbound"]}
          rows={byChannel.map((r: any) => [r.channel, r.outbound, r.inbound])}
        />
        <TableCard
          title="By Campaign"
          columns={["Campaign", "Outbound", "Inbound"]}
          rows={byCampaign.map((r: any) => [r.campaign_id, r.outbound, r.inbound])}
        />
      </div>
    </div>
  );
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td className="py-3 text-gray-600" colSpan={columns.length}>
                  No data
                </td>
              </tr>
            )}
            {rows.map((r, idx) => (
              <tr key={idx} className="border-t">
                {r.map((v, i) => (
                  <td key={i} className="py-2 pr-4 text-gray-800">
                    {String(v)}
                  </td>
                ))}
              </tr>
